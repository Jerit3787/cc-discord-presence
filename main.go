...
// Model pricing per million tokens. Keys are normalized model IDs (any
// trailing -YYYYMMDD snapshot date is stripped before lookup, so
// "claude-sonnet-4-5-20241022" and "claude-sonnet-4-5" share an entry).
// Unknown models fall back to formatModelName's tier heuristic + defaultPricing.
// Update when new models are released: https://www.anthropic.com/pricing
...
// defaultPricing is used when a model ID matches no known entry.
